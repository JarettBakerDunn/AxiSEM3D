...
// constructor
StreamSTF::StreamSTF(
    const std::string& fileName, PaddingMode padding, numerical::Real left, numerical::Real right) :
    mFileName(fileName), mPadding(padding != PaddingMode::None), mLeftPadding(left),
    mRightPadding(right) {
  // open and read
  ...
  for (const std::string& line : lines) {
    ...
    // size check
    if (words.size() != 2) {
      throw std::runtime_error("StreamSTF::StreamSTF || "
                               "Input file for STF must contain two columns."
                               " || Ascii file: " +
          io::popInputDir(mFileName));
    }
    // valid line
    mTimes.push_back(bstring::cast<double>(words[0], "StreamSTF::StreamSTF"));
    mData.push_back(bstring::cast<numerical::Real>(words[1], "StreamSTF::StreamSTF"));
  }
...
// get value
numerical::Real
StreamSTF::getValue(double time) {
  try {
    // interpolate in time
    int index0 = -1, index1 = -1;
    double factor0 = 0., factor1 = 0.;
    vector_tools::linearInterpSorted(mTimes, time, index0, index1, factor0, factor1);
    return (numerical::Real)(factor0 * mData[index0] + factor1 * mData[index1]);
  } catch (...) {
    if (!mPadding) {
      throw std::runtime_error("StreamSTF::getValue || "
                               "Time is out of range with padding disabled."
                               " || Requested time = " +
          bstring::toString(time) +
          " || STF time range = " + bstring::range(mTimes.front(), mTimes.back()) +
...
StreamSTF::verbose() const {
  ...
  const axisem3d::eigen::RColX& d = Eigen::Map<const axisem3d::eigen::RColX>(mData.data(), mData.size());

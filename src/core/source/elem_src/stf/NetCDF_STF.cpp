...
// constructor
NetCDF_STF::NetCDF_STF(const std::string& fileName,
    ...
    mPadding(padding != PaddingMode::None), mLeftPadding(left), mRightPadding(right) {
  // file
  ...
  // check time size
  if (chunkSize < 2) {
    throw std::runtime_error("NetCDF_STF::NetCDF_STF || "
                             "Insufficient time points, at least 2."
                             " || NetCDF file: " +
        io::popInputDir(mFileName) + " || NetCDF variable for time: " + mVarTime +
  ...
  mVTmax = dataAll.back();
  const axisem3d::eigen::RColX& vals = Eigen::Map<const axisem3d::eigen::RColX>(dataAll.data(), dataAll.size());
  mVmin = vals.minCoeff();
  mVmax = vals.maxCoeff();
...
// get value
numerical::Real
NetCDF_STF::getValue(double time) {
  // check padding first
  if (mPadding) {
    if (time < mTmin) {
      return mLeftPadding;
  ...
  // check buffer
  while (time > mTimes[mTimeStepOfTimesLast - mTimeStepOfTimes0]) {
    if (!loadNextBufferChunk()) {
      throw std::runtime_error("NetCDF_STF::getValue || "
                               "Time is out of range with padding disabled."
                               " || Requested time = " +
          bstring::toString(time) + " || STF time range = " + bstring::range(mTmin, mTmax) +
          " || NetCDF file: " + io::popInputDir(mFileName) + " || NetCDF variable for time: " +
          mVarTime + " || NetCDF variable for data: " + mVarData);
    }
  }
  try {
    // interpolate in time
    ...
    vector_tools::linearInterpSorted(mTimes,
        time,
        index0,
        index1,
        factor0,
        factor1,
        0,
        mTimeStepOfTimesLast - mTimeStepOfTimes0 + 1);
    return (numerical::Real)(factor0 * mData[index0] + factor1 * mData[index1]);
  } catch (...) {
    throw std::runtime_error("NetCDF_STF::getValue || "
                             "Time is out of range with padding disabled."
                             " || Requested time = " +
        ...
        " || NetCDF variable for time: " + mVarTime + " || NetCDF variable for data: " + mVarData);
  }
}

// verbose
std::string
NetCDF_STF::verbose() const {
  ...
  // range
  ss << boxEquals(4, 18, "1st (time, value)", range(mTmin, mVTmin, '(', ')'));
  ss << boxEquals(4, 18, "last (time, value)", range(mTmax, mVTmax, '(', ')'));
...
bool
NetCDF_STF::loadNextBufferChunk() {
  // check remaining
  ...
  // read time
  sReaders.at(mFileName).readVariable(
      mVarID_Time, mVarTime, mTimes, {mTimeStepOfTimes0}, {readCount});
  sReaders.at(mFileName).readVariable(
      mVarID_Data, mVarData, mData, {mTimeStepOfTimes0}, {readCount});
  return true;
}

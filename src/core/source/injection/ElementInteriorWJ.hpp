...
template <class Element, int ndim> class ElementInteriorWJ : public Element {
  ...
  public:
  // constructor
  ElementInteriorWJ(const Element& elementBase, const std::vector<int>& pointsOnWJB) :
  ...
  void
  collectDisplFromPoints(vec_arD_CMatPP_RM& displElem) const {
    // base
    ...
    for (int alpha = 0; alpha < this->getNu_1(); alpha++) {
      for (int idim = 0; idim < ndim; idim++) {
         Eigen::Map<axisem3d::eigen::CRowN>(displElem[alpha][idim].data())(mPointsOnWJB) +=
            Eigen::Map<const axisem3d::eigen::CRowN>((*mInjectedDispl)[alpha][idim].data())(mPointsOnWJB);
      }
    }
  }
  ...
  void
  addStiffToPoints(vec_arD_CMatPP_RM& stiffElem) const {
    ...
    for (int alpha = 0; alpha < this->getNu_1(); alpha++) {
      for (int idim = 0; idim < ndim; idim++) {
         Eigen::Map<axisem3d::eigen::CRowN>(stiffElem[alpha][idim].data())(mPointsOnWJB) -=
             Eigen::Map<const axisem3d::eigen::CRowN>(sInteriorStiff[alpha][idim].data())(mPointsOnWJB);
      }
    }
  ...
  // set stf
  void
  setSTF(bool aligned, int bufferSize, const std::shared_ptr<NetCDF_Reader>& reader) {
    // stf
    mSTF = std::make_unique<STF_NetCDF>(aligned, bufferSize, reader, quadKey());

    // workspace
    if (sInjectedDisplFlat.rows() < mSTF->getPatternNu_1()) {
      sInjectedDisplFlat.resize(mSTF->getPatternNu_1(), spectral::nPEM * ndim);
    }
  }

  // apply
  void
  applyInjection(int timeStep, double time) const {

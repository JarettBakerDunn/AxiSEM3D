...
class SolidPoint : public Point {
  ...
  extractComm(const axisem3d::eigen::CColX& buffer, int& row) {
    mFields.mStiff += Eigen::Map<const axisem3d::eigen::CMatX3>(&buffer(row), mFields.mStiff.rows(), 3);
    row += mFields.mStiff.size();
  }
  ...
  doScanning(numerical::Real relTolFourierH2,
    ...
    if (mScanningS) {
      mScanningS->doScanning(
          relTolFourierH2, relTolH2, absTolH2, maxNumPeaks, mFields.mDispl.col(0));
      mScanningP->doScanning(
          relTolFourierH2, relTolH2, absTolH2, maxNumPeaks, mFields.mDispl.col(1));
      mScanningZ->doScanning(
          relTolFourierH2, relTolH2, absTolH2, maxNumPeaks, mFields.mDispl.col(2));
    }
  }

...
// constructor
FluidElement::FluidElement(int quadTag,
    std::unique_ptr<const GradQuad>& grad,
    std::unique_ptr<const PRT>& prt,
    std::unique_ptr<const Acoustic>& acoustic,
    const std::array<std::shared_ptr<FluidPoint>, spectral::nPEM>& points) :
    Element(quadTag, grad, prt), mAcoustic(acoustic.release()),
    mInFourier((mPRT ? mPRT->is1D() : true) && mAcoustic->is1D()), mPoints(points) {
  // construct derived
  constructDerived();
}
...
FluidElement::FluidElement(const FluidElement& other) :
    ...
    mInFourier((mPRT ? mPRT->is1D() : true) && mAcoustic->is1D()), mPoints(other.mPoints) {
  // construct derived
  constructDerived();
}
...
// get point
Point&
FluidElement::getPoint(int ipnt) const {
  return *(mPoints[ipnt]);
}
...
FluidElement::displToStiff(
    const axisem3d::eigen::vec_ar1_CMatPP_RM& displElem, axisem3d::eigen::vec_ar1_CMatPP_RM& stiffElem) const {
  // displ => strain
  ...
  if (mPRT) {
    ...
    if (mInFourier) {
      mPRT->sphericalToUndulated3_FR(sStrainSpherical_FR, sStrainUndulated_FR, mNu_1);
      mAcoustic->strainToStress_FR(sStrainUndulated_FR, sStressUndulated_FR, mNu_1);
      mPRT->undulatedToSpherical3_FR(sStressUndulated_FR, sStressSpherical_FR, mNu_1);
    } else {
      fft::gFFT_N3.computeC2R(sStrainSpherical_FR, sStrainSpherical_CD, mNr);
      mPRT->sphericalToUndulated3_CD(sStrainSpherical_CD, sStrainUndulated_CD, mNr);
      mAcoustic->strainToStress_CD(sStrainUndulated_CD, sStressUndulated_CD, mNr);
      mPRT->undulatedToSpherical3_CD(sStressUndulated_CD, sStressSpherical_CD, mNr);
      fft::gFFT_N3.computeR2C(sStressSpherical_CD, sStressSpherical_FR, mNr);
    }
    mTransform->transformRTZ_SPZ3(sStressSpherical_FR, mNu_1);
  } else {
    if (mInFourier) {
      mAcoustic->strainToStress_FR(sStrainSpherical_FR, sStressSpherical_FR, mNu_1);
    } else {
      fft::gFFT_N3.computeC2R(sStrainSpherical_FR, sStrainSpherical_CD, mNr);
      mAcoustic->strainToStress_CD(sStrainSpherical_CD, sStressSpherical_CD, mNr);
      fft::gFFT_N3.computeR2C(sStressSpherical_CD, sStressSpherical_FR, mNr);
    }
  }

  // stress => stiffness
  mGradQuad->computeQuad3(sStressSpherical_FR, stiffElem, mNu_1);
}
...
void
FluidElement::computeStiff() const {
  // collect displacement from points
...
// measure cost
double
FluidElement::measure(int count) const {
  // random displacement
...
void
FluidElement::preparePressureSource() const {
  for (int ipnt = 0; ipnt < nPEM; ipnt++) {
...
void
FluidElement::prepareWavefieldOutput(
    const channel::fluid::ChannelOptions& chops, bool enforceCoordTransform) {
  // pressure
...
FluidElement::getDisplField(axisem3d::eigen::CMatXN3& displ, bool needRTZ) const {
  ...
  ///////////////////// compute stress /////////////////////
  // displ => strain
  mGradQuad->computeGrad3(sDisplSpherical_FR, sStrainSpherical_FR, mNu_1);
  ...
  if (mPRT) {
    ...
    if (mInFourier) {
      ...
      mPRT->sphericalToUndulated3_CD(sStrainSpherical_CD, sStrainUndulated_CD, mNr);
      mAcoustic->strainToStress_CD(sStrainUndulated_CD, sStressUndulated_CD, mNr);
      fft::gFFT_N3.computeR2C(sStressUndulated_CD, sStressUndulated_FR, mNr);
    }
    if (!needRTZ) {

...
// initialize
void
StationIO_ParNetCDF::initialize(const std::string& groupName,
    int numRecordSteps,
  ...
  if (mpi::rank() == mRankWithMaxNumStations) {
    ...
    for (int irank = 0; irank < mpi::nproc(); irank++) {
      stKeysAll.insert(stKeysAll.end(), stKeysRanks[irank].begin(), stKeysRanks[irank].end());
    }
    ...
    for (int ich = 0; ich < channels.size(); ich++) {
      mNcFile->writeVariable(
          "list_channel", channels[ich], {ich, 0}, {1, (int)channels[ich].size()});
    }

    // list station
    for (int ist = 0; ist < stKeysAll.size(); ist++) {
      mNcFile->writeVariable(
          "list_station", stKeysAll[ist], {ist, 0}, {1, (int)stKeysAll[ist].size()});
    }

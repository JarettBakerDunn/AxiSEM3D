...
class ElementOp {
  ...
  template <int D,
      ...
      typename RMatXND_RM =
          Eigen::Matrix<numerical::Real, Eigen::Dynamic, spectral::nPEM * D, Eigen::RowMajor>>
  void
  recordToElem(CMatXND& cxnd,
    ...
    if (npnts != spectral::nPEM) {
      for (int dim = 0; dim < D; dim++) {
        cxnd.block(0, npnts * dim, nu_1, npnts) =
            cxnd.block(0, spectral::nPEM * dim, nu_1, spectral::nPEM)(Eigen::all, mIPnts);
      }
    }
    ...
    if (nphis == 0) {
      ...
      for (int alpha = 1; alpha < nu_1; alpha++) {
        rxnd.block(alpha * 2 - 1, 0, 1, npntsD) = cxnd.block(alpha, 0, 1, npntsD).real();
        rxnd.block(alpha * 2 - 0, 0, 1, npntsD) = cxnd.block(alpha, 0, 1, npntsD).imag();
      }
    } else {
      // Fourier interpolation
      for (int iphi = 0; iphi < nphis; iphi++) {
        eigen_tools::computeFourierAtPhiExp(cxnd, nu_1, expIAlphaPhi.col(iphi), rxnd, iphi, npntsD);
      }
    }
    ...
    field.slice(axisem3d::eigen::IArray4({0, 0, 0, bufferLine}), axisem3d::eigen::IArray4({na, npnts, D, 1}))
        .reshape(axisem3d::eigen::IArray3{na, npnts, D}) = // the longest C++ statement I have written
      ...
      if (D == 3) {
        ...
        if (fieldIndex == -1) {
          ...
          for (int dim = 0; dim < 3; dim++) {
            ...
            res -= (field.slice(loc4, len4).reshape(copy) * field.slice(lox4, len4).reshape(copy));
          }

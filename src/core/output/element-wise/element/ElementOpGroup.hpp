...
template <class ElementOpT> class ElementOpGroup {
  ...
  ElementOpGroup(const std::string& groupName,
      ...
      mGroupName(groupName), mNumRecordSteps(numRecordSteps), mSampleIntv(sampleIntv), mTmin(tmin),
      mTmax(tmax), mDumpIntv(std::min(dumpIntv, numRecordSteps)),
      mChannelOptions(wcs, userChannels), mNPnts(npnts), mPhis(phis), mNaSpace(naSpace),
      mIO(io.release()) {
    // nothing
  }
  ...
  // add an element
  void
  addElementOp(std::unique_ptr<ElementOpT>& elementOp) {
  ...
  initialize() {
    ...
    for (int ich = 0; ich < nch; ich++) {
      const std::string& chstr = std::get<0>(
          ElementOpInference<ElementOpT>::gChannelMap.at(mChannelOptions.mStdChannels[ich]));
    ...
    for (int irank = 0; irank < mpi::nproc(); irank++) {
      mNaGrid.insert(mNaGrid.end(), naGridGlobal[irank].begin(), naGridGlobal[irank].end());
    }
    vector_tools::sortUnique(mNaGrid);
    ...
    for (int ielem = 0; ielem < nelem; ielem++) {
      ...
      auto it = std::lower_bound(mNaGrid.begin(), mNaGrid.end(), allNa[ielem]);
    ...
    for (int inag = 0; inag < mNaGrid.size(); inag++) {
      axisem3d::eigen::RTensor5 bufferField(numElemNaGrid[inag], mNaGrid[inag], mNPnts, nch, mDumpIntv);
    ...
    // IO
    mIO->initialize(
        mGroupName, mNumRecordSteps, channels, mNPnts, mNaGrid, mElemNaInfo, elemCoords);
  ...
  void
  record(int timeStep, double time) {
  ...
  dumpToFile() {
    ...
    for (int ie = 0; ie < mElementOps.size(); ie++) {
      ...
      mElementOps[ie]->processReport(
          mBufferLine, mChannelOptions, elemIndexNaGrid, naGridIndex, mBufferFields);
    }
  ...
  // channels
  const typename ElementOpInference<ElementOpT>::ChannelOptions mChannelOptions;

...
Model3D::computeElemSPZ(const Quad& quad, bool undulated) {
  ...
  for (int ipnt = 0; ipnt < spectral::nPEM; ipnt++) {
    ...
    spz.block(row, 1, nr, 1) = axisem3d::eigen::DColX::LinSpaced(nr, 0, 2. * numerical::dPi / nr * (nr - 1));
    row += nr;
  }
...
// build from inparam
void
Model3D::buildInparam(const ExodusMesh& exodusMesh,
  ...
  for (int mindex = 0; mindex < modelCount; mindex++) {
    ...
    const std::string& modelName = inparam::gInparamModel.get<std::string>(
        keyInparam + ":{" + bstring::toString(mindex) + "}");
    ...
    std::shared_ptr<const Model3D> model =
        Volumetric3D::buildInparam(exodusMesh, localMesh, modelName, keyInparam);
    // try geometric
    if (model == nullptr) {
      model = Geometric3D::buildInparam(exodusMesh, localMesh, modelName, keyInparam);
    }

    // try ocean load
    if (model == nullptr) {
      model = OceanLoad3D::buildInparam(exodusMesh, localMesh, modelName, keyInparam);
    }

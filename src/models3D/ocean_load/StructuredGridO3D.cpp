...
// constructor
StructuredGridO3D::StructuredGridO3D(const std::string& modelName,
    ...
    mSourceCentered(sourceCentered), mXY(xy), mEllipticity(ellipticity), mDataVarName(dataVarName),
    mFactor(factor), mSuperOnly(superOnly) {
  ////////////// init grid //////////////
  // info
...
// verbose
std::string
StructuredGridO3D::verbose() const {
  if (!mpi::root()) {

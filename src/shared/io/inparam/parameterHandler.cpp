 ...
 */
void
inparam::ParameterHandler::declareParameter(std::vector<std::string> keywords,std::string defaultValue, Yaml::Node& node) {
    /*
    ...
    */
    if (keywords.size()==1){
        ...
        declareParameter(keywords,defaultValue,childNode);
    }
}

void
inparam::ParameterHandler::declareParameter(std::string keyword,std::string defaultValue, Yaml::Node& node) {
    declareParameter(bstring::split(keyword, ":"),defaultValue,node);
}

...
class InparamYAML {
  public:
  // constructor
  InparamYAML(const std::string& name) : mName(name), mRoot() {
    // nothing
  }
  ...
  template <typename T>
  T
  get(const std::vector<std::string>& keyword) const {
    ...
    for (int depth = 0; depth < keyword.size(); depth++) {
      ...
      // array or object?
      if (keyword[depth].front() == '[' || keyword[depth].front() == '{') {
        ////////////////////// array //////////////////////
        // check sequence type
        if (!nodePtr->IsSequence() && nodePtr->As<std::string>() != "YAML_EMPTY_SEQUENCE") {
        ...
        if ((keyword[depth].front() == '[' && keyword[depth].back() != ']') ||
            (keyword[depth].front() == '{' && keyword[depth].back() != '}')) {
          throw std::runtime_error("InparamYAML::get || "
        ...
        std::string indexStr = keyword[depth].substr(1, keyword[depth].size() - 2);
        ...
        if (indexStr == "?") {
          ...
          return bstring::cast<T>(bstring::toString(nodePtr->Size()), "InparamYAML::get");
        }
        ...
        if (index < nodePtr->Size()) {
          ...
          if (returnKey) {
            ...
            nodePtr = &((*iter).second);
            // return key
            return bstring::cast<T>((*nodePtr->Begin()).first, "InparamYAML::get");
          } else {
        ...
        if (nodePtr->IsMap() && nodePtr->As<std::string>() != "YAML_EMPTY_SEQUENCE") {
  ...
  template <typename T>
  T
  getWithOptions(const std::string& keyword, const std::map<std::string, T>& options) const {
  ...
  T
  getWithBounds(
      const std::string& keyword, T lower, T upper = std::numeric_limits<T>::max()) const {
    T res = get<T>(keyword);
    if (res < lower || res > upper) {
      const std::string& range = bstring::range(lower, upper);
      throw std::runtime_error("InparamYAML::getWithBounds || " + keyword + " ∉ " + range +
          " || "
          "Keyword: " +
          keyword +
          " || "
  ...
  template <typename T>
  std::vector<T>
  getVector(const std::string& keyword) const {
    // get size
  ...
  // check existence
  bool
  contains(const std::string& keyword) const {
    try {

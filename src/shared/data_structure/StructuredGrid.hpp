...
template <int D, typename T> class StructuredGrid {
  public:
  // constructor
  ...
  StructuredGrid(const std::string& ncFile,
    ...
    for (auto it = dataInfo.begin(); it != dataInfo.end(); ++it) {
      int loc = (int)std::distance(uniqueDataVarNames.begin(), uniqueDataVarNames.find(it->first));
    ...
    // memory info
    timer::gPreloopTimer.message(eigen_tools::memoryInfoTensor(mGridData, "unique grid data"));
  ...
  bool
  inScope(const ArrayCS& crdTarget) const {
    // linear interp 0-1 points
    int index0 = -1, index1 = -1;
    double factor0 = 0., factor1 = 0.;
    for (int idim = 0; idim < D; idim++) {
      try {
        vector_tools::linearInterpSorted(
            mGridCoords[idim], crdTarget[idim], index0, index1, factor0, factor1);
      } catch (...) {
  ...
  template <typename ArrayCS>
  TRowV
  compute(const ArrayCS& crdTarget, const TRowV& valOutOfRange, int& dimOutOfRange) const {
    // linear interp 0-1 points
    static std::array<std::array<int, D>, 2> index01;
    static std::array<std::array<double, D>, 2> factor01;
    for (int idim = 0; idim < D; idim++) {
      try {
        vector_tools::linearInterpSorted(mGridCoords[idim],
            crdTarget[idim],
            index01[0][idim],
            index01[1][idim],
            factor01[0][idim],
  ...
  template <typename ArrayCS>
  TRowV
  compute(const ArrayCS& crdTarget, const TRowV& valOutOfRange) const {
  ...
  template <typename ArrayCS>
  T
  compute(const ArrayCS& crdTarget, T valOutOfRange, int varIndex = 0) const {
  ...
  // get grid coords
  const std::array<std::vector<double>, D>&
  getGridCoords() const {
    return mGridCoords;
  }

  // get grid coords
  std::array<std::vector<double>, D>&
  getGridCoords() {
    return mGridCoords;
  }

  // get grid data
  const Eigen::Tensor<T, 1 + D, Eigen::RowMajor>&
  getGridData() const {
    return mGridData;
  }

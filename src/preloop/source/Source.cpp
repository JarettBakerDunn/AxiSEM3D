...
Source::buildInparam(int sindex) {
  ...
  if (sindex == 0 &&
      inparam::gInparamModel.get<std::string>("geodesy:lat_lon_north_pole_mesh") == "SOURCE") {
    axial = true;
  }
  ...
  return std::make_shared<const Source>(
      sourceName, axial, sourceCentered, ellipticity, useDepth, depthSolid, undulated, crd);
}
...
Source::computeSPZ(const SE_Model& sem,
  ...
  if (useDepth) {
    double R = (depthSolid ? geodesy::getOuterSolidRadius() : geodesy::getOuterRadius());
    crd(2) = R - crd(2);
  }
    ...
    if (sourceCentered) {
      ...
      if (geodesy::isCartesian()) {
        ...
        if (spzUnbend(2) < spzUnbend(0) * 10.) {
          throw std::runtime_error("Source::computeSPZ || "
...
// verbose
std::string
Source::verbose(int sindex, const STF& stf, const Mechanism& mechanism) const {
  // title
  ...
  if (mUseDepth) {
    if (mDepthSolid) {
      ss << boxEquals(4, width, "depth", toString(mCrdIn(2)) + " (below solid surface)");
    } else {
      ss << boxEquals(4, width, "depth", toString(mCrdIn(2)) + " (below outer surface)");
    }
  } else {
...
void
Source::release(const SE_Model& sem, Domain& domain, double dt, double& minT0) {
  // source count
  ...
  for (int sindex = 0; sindex < sourceCount; sindex++) {
    ...
    const axisem3d::eigen::DRow3& spz = computeSPZ(sem,
        ...
        "Source name: " + source->mName,
        source->mAxial);

    // get solid/fluid from mechanism
    std::shared_ptr<const Mechanism> mechanism = Mechanism::buildInparam(sindex, source->mName);
  ...
  if (sourceIndexRank.size() != sourceCount && mpi::root()) {
    for (int sindex = 0; sindex < sourceCount; sindex++) {
      if (sourceIndexRank.find(sindex) == sourceIndexRank.end()) {
        throw std::runtime_error("Source::release || Error locating source in mesh. || "
                                 "Source index = " +
            bstring::toString(sindex) +
            " || "
            "Source name  = " +
            sources[sindex]->mName);
      }
    }
  }
  ...
  for (int sindex = 0; sindex < sourceCount; sindex++) {
    ...
    const axisem3d::eigen::DRowN& inplaneFactor = sem.computeInplaneFactor(spz({0, 2}).transpose(), quadTag);
    ...
    if (io::gVerbose != io::VerboseLevel::None) {
      verboseRank.insert({sindex, source->verbose(sindex, *stf, *mechanism)});
    }

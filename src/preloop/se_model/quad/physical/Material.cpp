...
////////////////////////// input //////////////////////////
// constructor
Material::Material(const ExodusMesh& exodusMesh, const axisem3d::eigen::DMat24& nodalSZ, bool axial) {
...
Material::getPointwiseRhoVpVs(
    axisem3d::eigen::arN_DColX& rho, axisem3d::eigen::arN_DColX& vp, axisem3d::eigen::arN_DColX& vs) const {
  // density
  ...
  if (currentRheology() == RheologyType::FLUID) {
    ...
    // vs = 0
    vs = NodalPhysicalProperty(axisem3d::eigen::DRow4::Zero(), getProperty("RHO").axial()).getPointwise();
...
Material::getMass(
    const axisem3d::eigen::DRowN& integralFactor, const axisem3d::eigen::arN_DColX& jacobianPRT, bool fluid) const {
  // check fluid
...
Material::createElastic(
    const std::unique_ptr<const AttBuilder>& attBuilder, const axisem3d::eigen::DRow4& weightsCG4) const {
  // check fluid
  if (currentRheology() == RheologyType::FLUID) {
...
Material::createAnisotropic(
  ...
  if (attBuilder) {
    ...
    axisem3d::eigen::DMatXN mu = (C11 + C22 + C33 - (C12 + C23 + C13) + (C44 + C55 + C66) * 3.) / 15.;
    C11 -= (kp + 4. / 3. * mu);
    C22 -= (kp + 4. / 3. * mu);
    C33 -= (kp + 4. / 3. * mu);
    C12 -= (kp - 2. / 3. * mu);
...
Material::createTISO(
  ...
  if (attBuilder) {
    ...
    // 1D or 3D
    elastic1D = op1D_3D::regularize1D<axisem3d::eigen::DMatXN>({ref(A), ref(C), ref(F), ref(L), ref(N)});
  }
...
Material::createIsotropic(
  ...
  if (attBuilder) {
    ...
    axisem3d::eigen::DMatXN QKp = getElemental("QKAPPA");
    axisem3d::eigen::DMatXN QMu = getElemental("QMU");
    elastic1D = op1D_3D::regularize1D<axisem3d::eigen::DMatXN>({ref(lambda), ref(mu), ref(QKp), ref(QMu)});

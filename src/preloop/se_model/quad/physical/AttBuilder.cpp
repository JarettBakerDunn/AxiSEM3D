...
AttBuilder::setAlphaBetaGamma() const {
  ...
  // beta
  const axisem3d::eigen::DColX& betta = yUn.cwiseProduct((nSLS1 - alpha).cwiseQuotient(wDt) - alpha);
  // gamma
  const axisem3d::eigen::DColX& gamma = yUn.cwiseProduct((alpha - nSLS1).cwiseQuotient(wDt) + nSLS1);

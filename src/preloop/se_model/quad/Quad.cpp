...
Quad::Quad(
  ...
  if (useLuckyNumbers) {
    (*mPointNr) = (*mPointNr).unaryExpr([](int nr) { return NrField::nextLuckyNumber(nr); });
  }
...
Quad::setupGLL(const ABC& abc, const LocalMesh& localMesh, std::vector<GLLPoint>& GLLPoints) const {
  ...
  if (abc.clayton()) {
    for (const std::string& key : abc.getBoundaryKeys()) {
      ...
      for (int ip = 0; ip < spectral::nPED; ip++) {
        int ipnt = ipnts[ip];
        int igll = localMesh.mElementGLL(mLocalTag, ipnt);
        GLLPoints[igll].addClaytonABC(key, mFluid, nABC[ip], rho[ipnt], vp[ipnt], vs[ipnt]);
      }
    }
  }
  ...
  if (abc.sponge()) {
    ...
    for (int ipnt = 0; ipnt < spectral::nPEM; ipnt++) {
      ...
      for (const std::string& key : abc.getBoundaryKeys()) {
        ...
        if (!mFluid) {
          gammaMax = abc.getU0Solid(std::abs(span), vp[ipnt], vs[ipnt], rho[ipnt]);
        } else {
          gammaMax = abc.getU0Fluid(std::abs(span), vp[ipnt], rho[ipnt]);
        }
        gammaMax *= pattern;
      }
      // release
  ...
  // axial
  if (axial()) {
    const std::vector<int>& ipnts = vicinity::constants::gEdgeIPnt[mEdgesOnBoundary.at("LEFT")];
    for (int ipnt : ipnts) {
      int igll = localMesh.mElementGLL(mLocalTag, ipnt);
  ...
  if (mEdgesOnBoundary.at("TOP") != -1) {
    const std::vector<int>& ipnts = vicinity::constants::gEdgeIPnt[mEdgesOnBoundary.at("TOP")];
    for (int ipnt : ipnts) {
      int igll = localMesh.mElementGLL(mLocalTag, ipnt);
  ...
  for (int nr = 0; nr < nrMax; nr++) {
    ...
    for (int ipnt0 = 0; ipnt0 < spectral::nPEM - 1; ipnt0++) {
      ...
      for (int ipnt1 = ipnt0 + 1; ipnt1 < spectral::nPEM; ipnt1++) {
        ...
        if (std::abs(ipol1 - ipol0) <= 1 && std::abs(jpol1 - jpol0) <= 1) {
          hmin = std::min(hmin, (sz.col(ipnt1) - sz.col(ipnt0)).norm());
        }
      }
    }
    // dt
...
// release to domain
void
Quad::release(const LocalMesh& localMesh,
  ...
  if (mFluid) {
    ...
    // element
    mFluidElement = std::make_shared<FluidElement>(mGlobalTag, grad, prt, acoustic, points);
    ...
    // element
    mSolidElement = std::make_shared<SolidElement>(mGlobalTag, grad, prt, elastic, points);
...
Quad::computeIntegralFactor(axisem3d::eigen::DMat2N& sz, std::array<axisem3d::eigen::DMat22, spectral::nPEM>& J) const {
  // xieta and weights
...
Quad::computeWeightsCG4(const axisem3d::eigen::DMatPP_RM& ifPP) const {
  ...
  wcg4(1) = (ifPP(0, 3) + ifPP(0, 4) + ifPP(1, 3) + ifPP(1, 4) +
                0.50 * (ifPP(0, 2) + ifPP(1, 2) + ifPP(2, 3) + ifPP(2, 4)) + 0.25 * ifPP(2, 2)) /
      ifPP(1, 3);
  wcg4(2) = (ifPP(3, 0) + ifPP(3, 1) + ifPP(4, 0) + ifPP(4, 1) +
                0.50 * (ifPP(2, 0) + ifPP(2, 1) + ifPP(3, 2) + ifPP(4, 2)) + 0.25 * ifPP(2, 2)) /
      ifPP(3, 1);
  wcg4(3) = (ifPP(3, 3) + ifPP(3, 4) + ifPP(4, 3) + ifPP(4, 4) +
                0.50 * (ifPP(2, 3) + ifPP(2, 4) + ifPP(3, 2) + ifPP(4, 2)) + 0.25 * ifPP(2, 2)) /
      ifPP(3, 3);
  return wcg4;
}

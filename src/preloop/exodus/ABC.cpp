...
// build from inparam
std::unique_ptr<ABC>
ABC::buildInparam(const ExodusMesh& exodusMesh) {
  ...
  if (abc->mSponge) {
    ...
    // read expr string
    abc->mGammaExprSolidStr = gm.get<std::string>(root + ":gamma_expr_solid");
    abc->mGammaExprFluidStr = gm.get<std::string>(root + ":gamma_expr_fluid");
  }
...
ABC::verbose() const {
  ...
  if (mSponge) {
    ...
    for (const std::string& key : mBoundaryKeys) {
      ...
      ss << boxEquals(4, 21, "range of sponge layer", range(outer - span, outer));
    }
...
ABC::getU0Solid(
    double span, const axisem3d::eigen::DColX& vp, const axisem3d::eigen::DColX& vs, const axisem3d::eigen::DColX& rho) const {
  // initialize
  int nr = (int)vp.rows();
...
// get U0 fluid
axisem3d::eigen::DColX
ABC::getU0Fluid(double span, const axisem3d::eigen::DColX& vp, const axisem3d::eigen::DColX& rho) const {
  // initialize
  int nr = (int)vp.rows();
  axisem3d::eigen::DColX U0 = axisem3d::eigen::DColX::Zero(nr, 1);
  // compute by expr

...
///////////////// construction /////////////////
// constructor
ExodusMesh::ExodusMesh(const std::string& meshFile) : mFileName(meshFile) {
  // memory info
  double memSup = 0.;
  double memAll = 0.;
...
ExodusMesh::verbose() const {
  ...
  ss << boxEquals(2, 20, "storage type", mElementNodesStorage ? "element_nodes" : "elements");
  ...
  for (auto it = mRadialVariables.begin(); it != mRadialVariables.end(); ++it) {
    const std::string& prange = range(it->second.minCoeff(), it->second.maxCoeff());
  ...
  for (auto it = mSideSets.begin(); it != mSideSets.end(); ++it) {
    ss << boxEquals(2, 20, it->first, toString(it->second.size()) + " sides", ":");
  }

  // miscellaneous (currently only ellipticity)
  ss << boxSubTitle(0, "Miscellaneous");
  ss << boxEquals(2, 20, "ellipticity curve", toString(mEllipticityCurve.cols()) + " knots", ":");
...
ExodusMesh::readBcastGlobal(const NetCDF_Reader& reader, double& memSup, double& memAll) {
  ...
  for (int igv = 0; igv < glbVarNames.size(); igv++) {
    ...
    if (glbVarNames[igv].find("w_") == 0 || glbVarNames[igv].find("y_") == 0 ||
...
// connectivity
void
ExodusMesh::readBcastConnectivity(const NetCDF_Reader& reader, double& memSup, double& memAll) {
  mpi::enterSuper();
  if (mpi::super()) {
    // read
    if (mpi::root()) {
    ...
    timer::gPreloopTimer.message(
        eigen_tools::memoryInfo(mySuperOnly().mConnectivity, "connectivity (super-only)", memSup));
  }
  mpi::enterWorld();
}
...
// coordinates
void
ExodusMesh::readBcastCoordinates(const NetCDF_Reader& reader, double& memSup, double& memAll) {
  mpi::enterSuper();
  if (mpi::super()) {
    // read
    if (mpi::root()) {
    ...
    // memory info
    timer::gPreloopTimer.message(
        eigen_tools::memoryInfo(mySuperOnly().mNodalCoords, "coordinates (super-only)", memSup));
  }
  mpi::enterWorld();
}
...
// side sets
void
ExodusMesh::readBcastSideSets(const NetCDF_Reader& reader, double& memSup, double& memAll) {
  // names
  ...
  for (int iss = 0; iss < ssNames.size(); iss++) {
    ...
    // memory info
    timer::gPreloopTimer.message(eigen_tools::memoryInfo(elems, ssNames[iss], memAll, 2));
    ...
    for (int iquad = 0; iquad < getNumQuads(); iquad++) {
      ...
      if (getAxialSide(iquad) >= 0) {
        for (int pA = 0; pA < 4; pA++) {
          int pB = (pA == 3) ? 0 : pA + 1;
          const auto& szA = mySuperOnly().mNodalCoords.row(mySuperOnly().mConnectivity(iquad, pA));
          const auto& szB = mySuperOnly().mNodalCoords.row(mySuperOnly().mConnectivity(iquad, pB));
...
// elemental variables
void
ExodusMesh::readBcastElemental(const NetCDF_Reader& reader, double& memSup, double& memAll) {
  mpi::enterSuper();
  if (mpi::super()) {
    // read
    if (mpi::root()) {
      ...
      exKey.str("");
      exKey << "vals_elem_var"
            << (int)(std::find(evNames.begin(), evNames.end(), "fluid") - evNames.begin() + 1)
            << "eb1";
      reader.readMatrix(exKey.str(), dbuffer);
    ...
    // memory
    timer::gPreloopTimer.message(
        eigen_tools::memoryInfo(mySuperOnly().mGeometryType, "element_type (super-only)", memSup));
    timer::gPreloopTimer.message(
        eigen_tools::memoryInfo(mySuperOnly().mIsElementFluid, "fluid (super-only)", memSup));
  }
  mpi::enterWorld();
}
...
// radial variables
void
ExodusMesh::readBcastRadial(const NetCDF_Reader& reader, double& memSup, double& memAll) {
  //////////////////////// name ////////////////////////
  ...
  bool isotropic =
      (evNameMap.find("ETA_0") == evNameMap.end() && evNameMap.find("ETA") == evNameMap.end());
  if (isotropic) {
    rvNames = std::vector<std::string>({"RHO", "VP", "VS"});
  } else {
    rvNames = std::vector<std::string>({"RHO", "VPV", "VPH", "VSV", "VSH", "ETA"});
  }
  if (hasAttenuation()) {
  ...
  if (mpi::root()) {
    ...
    for (int iquad = 0; iquad < getNumQuads(); iquad++) {
      ...
      if (axisSide >= 0) {
        ...
        double z1 = mySuperOnly().mNodalCoords(mySuperOnly().mConnectivity(iquad, axisSide), 1);
        double z2 = mySuperOnly().mNodalCoords(mySuperOnly().mConnectivity(iquad, otherNode), 1);
        ...
        quad_nodes.insert(quad_nodes.begin() + (ir1 - coords.begin()), {iquad, axisSide});
        auto ir2 = coords.insert(std::upper_bound(coords.begin(), coords.end(), z2), z2);
        quad_nodes.insert(quad_nodes.begin() + (ir2 - coords.begin()), {iquad, otherNode});
      }
    }
    ...
    for (int iname = 0; iname < rvNames.size(); iname++) {
      ...
      if (mElementNodesStorage) {
        ...
        for (int idep = 0; idep < coords.size(); idep++) {
          rvValues(idep, iname) = buf[quad_nodes[idep].second](quad_nodes[idep].first);
        }
      } else {
        // storage = elements
    ...
    for (int idep = 0; idep < coords.size() - 1; idep++) {
      ...
      for (int idisc = 0; idisc < mDiscontinuities.size(); idisc++) {
        if (mDiscontinuities[idisc] < coords[idep + 1] && mDiscontinuities[idisc] > coords[idep]) {
      ...
      if (!gapIsDisc) {
        ...
        rvValues.row(idep) = (rvValues.row(idep) + rvValues.row(idep + 1)) * .5;
  ...
  timer::gPreloopTimer.message(
      eigen_tools::memoryInfo(Eigen::Map<axisem3d::eigen::DColX>(mRadialCoords.data(), mRadialCoords.size()),
          "anchoring radii",
          memAll));
  ...
  for (int irv = 0; irv < rvNames.size(); irv++) {
    ...
    // memory infor
    timer::gPreloopTimer.message(eigen_tools::memoryInfo(rvValues.col(irv), rvNames[irv], memAll));
  }
  ...
  for (auto ita = mSideSets.at(mKeyLeftSS).begin(); ita != mSideSets.at(mKeyLeftSS).end(); ita++) {
    int iquad = ita->first;
    ...
    for (auto it = mSideSets.begin(); it != mSideSets.end(); ++it) {
      if (getSide(it->first, iquad) != -1) {
        it->second.at(iquad) = Mapping::cycle4(it->second.at(iquad) - axialSide + 3);
      }
    }
  }
}
...
// ellipticity
void
ExodusMesh::readEllipticity(const NetCDF_Reader& reader, double& memSup, double& memAll) {
  if (mpi::root()) {
    if (!isCartesian()) {
      // read
    ...
    timer::gPreloopTimer.message(eigen_tools::memoryInfo(mEllipticityCurve, "ellipticity", memAll));
  }
}
...
// form Nr at nodes
void
ExodusMesh::formNrAtNodes(const NrField& nrField, bool boundByInplane, bool useLuckyNumbers) {
  ...
  if (useLuckyNumbers) {
    ...
    nrMe = nrMe.unaryExpr([](int nr) { return NrField::nextLuckyNumber(nr); });
...
ExodusMesh::ExodusSuperOnly&
ExodusMesh::mySuperOnly() {
  if (!mpi::super()) {
...
const ExodusMesh::ExodusSuperOnly&
ExodusMesh::mySuperOnly() const {
  if (!mpi::super()) {
    throw std::runtime_error("ExodusMesh::mySuperOnly || "
                             "Access to super-only variables from "
                             "a non-super rank.");
...
bool
ExodusMesh::boundaryInfoABC(const std::string& boundaryKey, double& outer, double& meshSpan) const {
  // side name
  std::string ssName;

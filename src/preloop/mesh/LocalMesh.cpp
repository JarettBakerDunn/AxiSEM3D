...
LocalMesh::decomposeExodusMesh(
    const ExodusMesh& exodusMesh, const axisem3d::eigen::DColX& weights, axisem3d::eigen::IColX& elemRank) {
  // metis
  ...
  int minProc = (int)(std::min_element(objAll.begin(), objAll.end()) - objAll.begin());
  mpi::bcastEigen(elemRank, minProc);
  timer::gPreloopTimer.message(eigen_tools::memoryInfo(elemRank, "element mpi-rank (super-only)"));
}

// build local skeleton
void
LocalMesh::buildLocalSkeleton(const ExodusMesh& exodusMesh, const axisem3d::eigen::IColX& elemRank) {
  // gather world rank
  ...
  if (mpi::root()) {
    ...
    for (int rank = 0; rank < mpi::nproc(); rank++) {
      ...
      if (vL2GE.size() == 0) {
        throw std::runtime_error("LocalMesh::buildLocalSkeleton || "
...
void
LocalMesh::buildElementGLL_CommMPI(const ExodusMesh& exodusMesh, const axisem3d::eigen::IColX& elemRank) {
  // local GLL
  ...
  if (mpi::root()) {
    ...
    timer::gPreloopTimer.message(
        eigen_tools::memoryInfo(axisem3d::eigen::IColX(gNeighbSize), "global neighbourhood (super-only)"));
    ...
    // loop over inferior ranks
    for (int rank = 0; rank < mpi::nproc(); rank++) {
      const std::vector<int>& vecCommProcGLL = vicinity::buildCommMPI(vecL2G_Element[rank],
          globalNeighbours,
          elemRank,
          vecWorldRank[rank],
...
LocalMesh::verbose(const std::string& meshTitle,
  ...
  if (mpi::root()) {
    ...
    const axisem3d::eigen::IColX& crcol = Eigen::Map<const axisem3d::eigen::IColX>(crvec.data(), crvec.size());
    const axisem3d::eigen::IColX& cgcol = Eigen::Map<const axisem3d::eigen::IColX>(cgvec.data(), cgvec.size());
...
LocalMesh::plotDD(const std::string& fname, const axisem3d::eigen::DColX& weights) const {
  ...
  if (mpi::root()) {
    ...
    // coords
    nw.defineVariable("coords", {{"dim_elem", nelem}, {"dim_sz", 2}}, numerical::dErr);
    nw.writeWholeVariable("coords", center);
    // weights
    nw.defineVariable("weights", {{"dim_elem", nelem}}, numerical::dErr);
    nw.writeWholeVariable("weights", weights);
    // rank
    nw.defineVariable("mpi_rank", {{"dim_elem", nelem}}, (int)-1);

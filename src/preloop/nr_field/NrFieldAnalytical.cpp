...
NrFieldAnalytical::NrFieldAnalytical() {
  // check code ID
  const std::string& codeID = inparam::gInparamNr.get<std::string>("analytical:code_ID");
  if (sCodeID != codeID) {
...
NrFieldAnalytical::getNrAtPoints(const axisem3d::eigen::DMatX2_RM& sz) const {
  ...
  [[maybe_unused]]
  const axisem3d::eigen::DColX& depth = geodesy::isCartesian() ? geodesy::getOuterRadius() - z.array()
  ...
  nr = depth.unaryExpr([this](double _dep) {
    ...
    try {
      vector_tools::linearInterpSorted(
          this->mControlDepths, _dep, index0, index1, factor0, factor1);
    } catch (...) {
    ...
    return (int)round(this->mControlNrs[index0] * factor0 + this->mControlNrs[index1] * factor1);
  });
...
// verbose
std::string
NrFieldAnalytical::verbose() const {
